...
#라벨로 접근할수 있게 한다. loc
a = temp.loc[temp['LdCountry'] == country]


b = a[['LdCountry','LPort' ,'Kg']]

print(b)

Lport_list = b.LPort.unique()


print(Lport_list)
kg_list = [0 for i in range(len(Lport_list))]


for i in range(len(kg_list)):

    c = b.loc[b['LPort'] == Lport_list[i]]
    kg_list[i] = c['Kg'].sum()

d = {'LdCountry' : country, 'LPort' :Lport_list, 'Kg': kg_list}

df = pd.DataFrame(data=d)

print(df)

export_csv = df.to_csv('./export_spain.csv')

#print(a[['LdCountry','LPort' ,'Kg']])

#print(a.loc[a['LPort'] == 'KRINC'])
